...
<div class="wrap">
  ...
  <p class="sub">A new investor has subscribed to the newsletter.</p>
  <table>
    <tr><td>Email</td><td><strong>{{ $subscriber->email }}</strong></td></tr>
    <tr><td>Source Page</td><td>{{ ucfirst($subscriber->source) }}</td></tr>
  </table>
  <p class="footer">Submitted {{ $subscriber->created_at->format('d M Y, H:i') }} UTC &nbsp;·&nbsp; ETAXPLANNER PROPERTY CONNECT</p>
</div>
</body></html>
